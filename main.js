...
// 1) nomreleri * kimi yazan function qurun yeni her elementin  "phone": "+44 254-761-6843" 
// meselen burda "phone": "+44 ***-***-**43" her elementin phonesi bele olsun
function maskPhoneNumbers(array) {
    return array.map(item => {
      let phone = item.phone;
      let maskedPhone = phone.replace(/(\d{2})(\d{3})(\d{3})(\d{2})(\d{2})/, '+44 ***-***-**43');
      return {...item, phone: maskedPhone};
    });
  }
  let maskedContacts = maskPhoneNumbers(users);
  console.log(maskedContacts);


  // 2) ele bir function qurun ki username daxil oldugunda o insanin phonesini versin
  function numberUsername(array, username) {
    let user = array.find(item => item.username === username);
    if (user) {
      return user.phone;
    }
    return null; 
  }
  let phone = numberUsername(users);
  console.log(phone);
  
  // 3) dogum ili 1990 ci ilden yuxari olanlari secen function yaradin
  function filterBirthYear(array) {
    return array.filter(item => item.birthYear.toString().slice(0, 4) >= "1990");
  }
  let filteredUsers = filterBirthYear(users);
  console.log(filteredUsers);

  // 4) butun emaillerin sonu @gmail.com ile bitmesini temin eden function yazin
// meselen  "email": "michael.williams@x.dummyjson.com", bele bir email varsa sonu "email": "michael.williams@gmail.com" 
// olaraq bitsin
function emailsToGmail(array) {
    return array.map(item => {
      if (item.email.slice(-10) !== '@gmail.com') {
        item.email = item.email.split('@')[0] + '@gmail.com';
      }
      return item;
    });
  }
  let updatedUsers = emailsToGmail(users);
  console.log(updatedUsers);
  
  // 5) eyni goz rengine uygun nece insan oldugunu tapan func yazin  bir array  daxilinde arrayde olan 
// butun goz renglerinin ne qeder insanda hansi goz rengine hansi goz renginin ne qeder insanda olmasini veren array yazin-?